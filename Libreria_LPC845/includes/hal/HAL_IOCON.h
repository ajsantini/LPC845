 ...
 * del mismo, se leerá como **cero** cuando externamente esté a una tensión de aproximadamente VSS y se
 * leerá como **uno** cuando externamente esté a una tensión de aproximadamente VDD. Si se
 * activa la inversión del pin sucederá lo inverso, es decir, si está configurado como entrada y externamente hay
 * una tensión de aproximadamente VSS la lectura se tomará como **uno** y si hay una tensión de
 * aproximadamente VDD la lectura se tomará como **cero**.
 *
 ...
 * *pull-up* o de *pull-down*. Esto implica que aunque la entrada no tenga ningún valor fuerte
 * externamente, estas resistencias se encargarán de fijar un valor o bien naturalmente alto (*pull-up*) o
 * bajo (*pull-down*). Existe un tercer tipo llamado *repetidor* el cual configura un
 * *pull-up* o un *pull-down* dependiendo del último valor que hubo en la entrada, esto quiere
 * decir que si el pin tiene un valor externamente fijado en estado alto, y queda sin un valor fijo, el
 * microcontrolador automáticamente dejará configurado un *pull-up*, de esta forma reteniendo el último
 * valor fijo, y viceversa cuando el pin es fijado externamente en estado bajo.
 *
 * # Salidas de tipo *Open Drain*
 *
 * Una salida tipo *open drain* en comparación con una salida tradicional, permite colocar un nivel
 * de tensión distinto al de la alimentación del microcontrolador cuando se acciona el pin.
 *
 * **NOTA**: La hoja de datos aclara que la tensión a la cual se puede manejar el pin mediante un resistor de
 * *pull-up* externo, no puede superar la tensión VDD.
 *
 * # Histéresis
 *
 ...
 * configurando correctamente el clock asociado al periférico *IOCON* mediante el @ref SYSCON, y el filtro de
 * *glitches* asociado al pin, el cual tomará una lectura como válida luego de una cantidad configurable de
 ...
 * dicho pin como salida analógica también mediante el periférico *IOCON*. Además se debe inhabilitar cualquier
 * tipo de resistor interno cuando se configura el pin como salida analógica.
 *
 * # Modo IIC
 *
 * Cuando se utiliza el periférico @ref IIC, en particular la instancia 0, se debe configurar mediante el *IOCON*
 * dependiendo de la velocidad con la que se va a utilizar.
 *
 * # Implementación en la librería
 *
 * Es importante destacar que cada periférico inicializará el *IOCON* en el punto que lo necesite, y luego lo
 * desactivará para conservar energía. La única excepción es el periférico @ref GPIO el cual no genera ninguna
 * inicialización respecto del *IOCON*, por lo que el usuario deberá configurar (en caso de ser necesario) cada
...
typedef struct
{
	...
	uint8_t open_drain; /**< Modo *Open Drain*. Cualquier valor distinto de cero lo activa */
