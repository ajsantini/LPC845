...
typedef enum
{
	...
	STATES_WAITING_OPEN,
	STATES_CLOSING,
	STATES_WAITING_CLOSE
}states_en;

static void tick_callback(void);

static uint32_t waiting_counter = 0;
...
int main(void)
{
	...
	while(1)
	{
		...
		case STATES_CLOSING:
		{
			if(sensores_tope_get_state(SENSOR_1, SENSOR_CERRADO))
			{
				motores_action(MOTOR_1, MOTOR_FRENAR);
